...
// Role-based hiring stage templates
...
export const ROLE_BASED_HIRING_STAGES = {
  ...
  [ROLE_TYPES.PRODUCT]: [
    ...
    },
    {
      id: "product_sense",
      name: "Product Sense",
      description: "Product intuition and strategy discussion",
    },
    {
      id: "case_study",
      name: "Case Study",
  ...
  [ROLE_TYPES.SALES]: [
    ...
    },
    {
      ...
      description: "Sales skills and methodology evaluation",
    },
    {
      id: "role_play",
      name: "Role Play",
      description: "Sales role-playing exercise",
    },
    {
      id: "final_interview",
      name: "Final Interview",
      description: "Final round with sales leadership",
    },
    {
      id: "offer",
      name: "Offer Extended",
  ...
  [ROLE_TYPES.MARKETING]: [
    ...
    },
    {
      id: "portfolio_review",
      name: "Portfolio Review",
      description: "Marketing portfolio and campaign review",
    },
    {
      id: "strategy_interview",
      name: "Strategy Interview",
      description: "Marketing strategy and planning discussion",
    },
    {
      id: "final_interview",
      name: "Final Interview",
      description: "Final round with marketing leadership",
    },
    {
      id: "offer",
      name: "Offer Extended",
  ...
  [ROLE_TYPES.OPERATIONS]: [
    ...
    },
    {
      ...
      description: "Operations and process improvement discussion",
    },
    {
      id: "analytical_assessment",
      name: "Analytical Assessment",
      description: "Data analysis and problem-solving exercise",
    },
    {
      id: "final_interview",
      name: "Final Interview",
      description: "Final round with operations leadership",
    },
    {
      id: "offer",
      name: "Offer Extended",
...
// Default hiring stages - fallback for custom roles
...
// Helper function to get hiring stages for a role type
export const getHiringStagesForRole = (roleType) => {
  return ROLE_BASED_HIRING_STAGES[roleType] || DEFAULT_HIRING_STAGES;
};
...
export const sampleRequisitions = [
  {
    ...
    hiring_stages: ROLE_BASED_HIRING_STAGES[ROLE_TYPES.ENGINEERING],
  },
  {
    id: "req-002",
    ...
    positions_to_fill: 1,
    role_type: ROLE_TYPES.PRODUCT,
    hiring_stages: ROLE_BASED_HIRING_STAGES[ROLE_TYPES.PRODUCT],
  },
  {
    id: "req-003",
    title: "UX Designer",
    ...
    positions_to_fill: 1,
    role_type: ROLE_TYPES.DESIGN,
    hiring_stages: ROLE_BASED_HIRING_STAGES[ROLE_TYPES.DESIGN],
  },
];
...
export const getRequisitionStats = (requisitionId, candidates = sampleCandidates, requisitions = sampleRequisitions) => {
...
export const addInterviewToCandidate = (candidateId, interview) => {
  const candidateIndex = sampleCandidates.findIndex(c => c.id === candidateId);
  if (candidateIndex !== -1) {
    if (!sampleCandidates[candidateIndex].interviews) {
      sampleCandidates[candidateIndex].interviews = [];
    }
    sampleCandidates[candidateIndex].interviews.push({
      ...interview,
      id: `interview-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    });
    sampleCandidates[candidateIndex].updated_at = new Date().toISOString();
    return sampleCandidates[candidateIndex];
  }
  return null;
};

export const removeInterviewFromCandidate = (candidateId, interviewIndex) => {
  const candidateIndex = sampleCandidates.findIndex(c => c.id === candidateId);
  if (candidateIndex !== -1 && sampleCandidates[candidateIndex].interviews) {
    sampleCandidates[candidateIndex].interviews.splice(interviewIndex, 1);
    sampleCandidates[candidateIndex].updated_at = new Date().toISOString();
    return sampleCandidates[candidateIndex];
  }
  return null;
...
export const updateCandidateInterviews = (candidateId, interviews) => {
  const candidateIndex = sampleCandidates.findIndex(c => c.id === candidateId);
  if (candidateIndex !== -1) {
    sampleCandidates[candidateIndex].interviews = interviews;
    sampleCandidates[candidateIndex].updated_at = new Date().toISOString();
    return sampleCandidates[candidateIndex];
  }
  return null;
...
export const getInterviewStatistics = (candidateId) => {
  ...
  const totalRating = interviews.reduce((sum, interview) => sum + interview.rating, 0);
  const averageRating = totalRating / interviews.length;
  const latestInterview = interviews.reduce((latest, current) => {
    return new Date(current.date) > new Date(latest.date) ? current : latest;
  });
  const interviewTypes = [...new Set(interviews.map(i => i.type))];
